...
namespace ProyectoEquipoSimulacion
{
    public partial class Form1 : Form
    {
       ...
       /* private void btnPruebaPromedio_Click(object sender, EventArgs e)
        {
            ...
            double area= 0.50-(Double.Parse(txtNivelSignificancia.Text)/2);

            for (int i = 0; i < 30; i++)
            {
                for(int j= 0; j<11; j++)
                {
                    double area2 = Double.Parse(dataGridView2.Rows[i].Cells[j].Value.ToString());
                    if (area2 == area)
                    {
                        double columnatabla = Double.Parse(dataGridView2.Rows[i].Cells[j].OwningColumn.HeaderText);
                        double filatabla = Double.Parse(dataGridView2.Rows[i].Cells[0].Value.ToString());
                        double vtablas = columnatabla + filatabla;
                        
                        i = 30;
                        label10.Text = vtablas.ToString();
                    }
                }
            }
        ...
        */
        ...
        {
            if (tbA.TextLength == 0 || tbC.TextLength == 0 || tbM.TextLength == 0 || tbn.TextLength == 0 || tbXo.TextLength == 0)
            {
            ...
            else
            {
                ...
                Xo = 0;
                M = 0;
                n = 0; modulo = 0; m = 0; acumulador = 0; promedio = 0; Zo = 0;
